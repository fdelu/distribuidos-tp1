...
class Aggregator(Protocol[GenericJoinedTrip, OUT]):
...
class AggregationHandler(Generic[GenericJoinedTrip, OUT]):
    comms: AggregatorComms[GenericJoinedTrip | End, OUT]
    aggregator: Aggregator[GenericJoinedTrip, OUT]
    config: Config
    timer: Any | None
    ends_received: int
    count: int = 0
    ...
    def handle_record(self, record: GenericJoinedTrip):
